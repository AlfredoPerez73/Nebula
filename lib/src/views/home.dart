...
class _HomeScreenPageState extends State<HomeScreen> {
  ...
  final EntrenamientoController entrenamientoController = Get.find<EntrenamientoController>();
    ...
    body: Container(
      decoration: BoxDecoration(
        gradient: LinearGradient(
          ...
          colors: [
            _backgroundColor,
            Color.lerp(_backgroundColor, _primaryColor, 0.4) ?? _primaryColor.withOpacity(0.8),
...
BoxDecoration _enhancedCardDecoration({Color? accentColor, double opacity = 0.1}) {
...
Widget _buildEnhancedButton(String text, VoidCallback onPressed, Color color) {
  return Material(
    color: Colors.transparent,
    child: InkWell(
      onTap: onPressed,
                ...
                Obx(() => Row(
                      children: [
                        Container(
                          ...
                          child: Center(
                            child: Text(
                              authController.userModel.value?.nombre?.substring(0, 1).toUpperCase() ?? 'U',
                              style: TextStyle(
                                fontSize: 22,
                                fontWeight: FontWeight.bold,
                                color: _accentColor,
          ...
          Container(
            ...
            decoration: BoxDecoration(
              gradient: LinearGradient(
                ...
                colors: [
                  categoryColor,
                  Color.lerp(categoryColor, Colors.black, 0.3) ?? categoryColor,
                ],
              ),
            ),
            child: Column(
...
Widget _buildUserStatsCard() {
  ...
  return GetBuilder<EntrenamientoController>(
    builder: (controller) {
      ...
      int totalLogros = 0;
      int maxLogros = 0;
      String nivelExperiencia = authController.userModel.value?.nivelExperiencia?.toLowerCase() ?? "principiante";
      ...
      int metaCalorias = _getMetaCalorias(nivelExperiencia, authController.userModel.value?.peso ?? 70);
      ...
      if (controller.entrenamientos.isNotEmpty) {
        ...
        for (var entrenamiento in controller.entrenamientos) {
          for (var ejercicio in entrenamiento.ejercicios) {
            ...
            int tiempoEjercicio = ejercicio.series * int.parse(ejercicio.repeticiones) * 1 ~/ 1;
...
Widget _buildUserProgressCard() {
  ...
  return GetBuilder<EntrenamientoController>(
    builder: (controller) {
      ...
      String nivelActual = authController.userModel.value?.nivelExperiencia?.toLowerCase() ?? "principiante";
      ...
      for (var entrenamiento in controller.entrenamientos) {
        for (var ejercicio in entrenamiento.ejercicios) {
          totalRepeticiones += ejercicio.series * int.parse(ejercicio.repeticiones);
      ...
      double progresoRepeticiones = (totalRepeticiones / metaRepeticiones).clamp(0.0, 1.0);
...
Widget _buildEnhancedProgressTracker({
  ...
  Color changeColor = isPositiveChange 
      ? Colors.green 
      : (isGoal ? Colors.amber : Colors.red);
